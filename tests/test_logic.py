...
def test_bo1_rejects_a_pool_too_large_for_a_view():
...
# --------------------------------------------------------- timer safety -----
def test_final_timeout_pick_is_not_cancelled_by_its_own_cleanup():
    """Regression test for a bug in _TimedView._cancel(): when a per-turn timer
    fires and reaches its "we're done" branch, it used to call `self._cancel()`
    on its own currently-running task. Self-cancelling doesn't raise right away,
    but the *next* await after it does — silently aborting whatever comes after
    (persisting the final pick, advancing to the next match stage).
    """
    import asyncio

    from bot.core.views import _TimedView

    class _Probe(_TimedView):
        SECONDS = 0  # fire almost immediately

        def __init__(self):
            super().__init__()
            self.follow_up_ran = False

        async def on_timeout_step(self):
            # mirrors the "done" branches in DraftView/VetoView/CoinflipView:
            # cancel (a no-op today, buggy before the fix), then do async work
            # that depends on not being aborted.
            await self._cancel()
            await asyncio.sleep(0)
            self.follow_up_ran = True
...
def test_highest_peak_breaks_ties_randomly_not_by_queue_order():
    """Regression test: sorted() alone is stable, so two players tied on
    peak rank used to always resolve to whichever joined the queue first."""
    from bot.services.draft import choose_captains

    players = [
        {"user_id": 1, "peak_rank": "Immortal 2"},
        {"user_id": 2, "peak_rank": "Immortal 2"},
        {"user_id": 3, "peak_rank": "Iron 1"},
    ]
    winners = {choose_captains("highest_peak", players)[0] for _ in range(200)}
    assert winners == {1, 2}   # both tied players get to be captain A sometimes


def test_highest_rr_breaks_ties_randomly_not_by_queue_order():
    from bot.services.draft import choose_captains
    ...
    winners = {choose_captains("highest_rr", players)[0] for _ in range(200)}
    assert winners == {1, 2}


def test_has_enough_rank_data_needs_at_least_two_with_data():
    from bot.services.draft import has_enough_rank_data

    none = [{"user_id": 1}, {"user_id": 2}, {"user_id": 3}]
    one = [{"user_id": 1, "peak_rank": "Gold 2"}, {"user_id": 2}, {"user_id": 3}]
    two = [{"user_id": 1, "peak_rank": "Gold 2"}, {"user_id": 2, "peak_rank": "Iron 1"},
           {"user_id": 3}]
    assert not has_enough_rank_data("highest_peak", none)
    assert not has_enough_rank_data("highest_peak", one)
    assert has_enough_rank_data("highest_peak", two)

    none_rr = [{"user_id": 1}, {"user_id": 2}]
    one_rr = [{"user_id": 1, "cur_rr": 40}, {"user_id": 2}]
    two_rr = [{"user_id": 1, "cur_rr": 40}, {"user_id": 2, "cur_rr": 10}]
    assert not has_enough_rank_data("highest_rr", none_rr)
    assert not has_enough_rank_data("highest_rr", one_rr)
    assert has_enough_rank_data("highest_rr", two_rr)

    # every other method doesn't consume rank data, so it's never blocked
    for method in ("random", "manual", "volunteer", "vote"):
        assert has_enough_rank_data(method, none)

    # highest_wins_* is never blocked either: `wins` always exists (0 for a
    # fresh player), so an all-zero field is an expected random pick among
    # ties, not broken data the way an all-null rank field would be.
    assert has_enough_rank_data("highest_wins_peak", none)
    assert has_enough_rank_data("highest_wins_rr", none)


def test_choose_captains_by_wins_ranks_wins_first_then_the_tiebreak():
    from bot.services.draft import choose_captains

    players = [
        {"user_id": 1, "wins": 3, "peak_rank": "Gold 2", "cur_rr": 40},
        {"user_id": 2, "wins": 3, "peak_rank": "Platinum 1", "cur_rr": 10},
        {"user_id": 3, "wins": 1, "peak_rank": "Radiant", "cur_rr": 500},
    ]
    # player 3 has the best rank by far but fewer wins — wins outranks it.
    assert choose_captains("highest_wins_peak", players) == (2, 1)
    assert choose_captains("highest_wins_rr", players) == (1, 2)


def test_highest_wins_breaks_ties_randomly_not_by_queue_order():
    from bot.services.draft import choose_captains

    players = [
        {"user_id": 1, "wins": 5, "peak_rank": "Iron 1", "cur_rr": 10},
        {"user_id": 2, "wins": 5, "peak_rank": "Iron 1", "cur_rr": 10},
        {"user_id": 3, "wins": 0, "peak_rank": "Radiant", "cur_rr": 500},
    ]
    peak_winners = {choose_captains("highest_wins_peak", players)[0] for _ in range(200)}
    rr_winners = {choose_captains("highest_wins_rr", players)[0] for _ in range(200)}
    assert peak_winners == {1, 2}
    assert rr_winners == {1, 2}


def test_series_winner_needs_a_majority():
    from bot.services.draft import series_winner

    assert series_winner([]) is None                # no maps reported yet
    assert series_winner(["A", "B"]) is None         # force-ended, tied
    assert series_winner(["A"]) == "A"               # BO1
    assert series_winner(["A", "B", "A"]) == "A"     # BO3
...
# --------------------------------------------------- captain names in embeds ---
def test_member_name_prefers_the_display_name_falls_back_to_a_mention():
    """Embeds don't parse mention syntax, so a cached member's display name is
    used instead of a raw <@id> wherever possible."""
    from types import SimpleNamespace

    from bot.core.embeds import member_name

    cached = SimpleNamespace(display_name="Salta")
    guild = SimpleNamespace(get_member=lambda uid: cached if uid == 1 else None)

    assert member_name(guild, 1) == "Salta"
    assert member_name(guild, 2) == "<@2>"        # not cached on this guild
    assert member_name(None, 3) == "<@3>"          # no guild at all
