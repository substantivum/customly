...
# The indicators that survived the emoji cleanup: lifecycle dots, the config
# OK/warning marks, the ready-check answer marks and the lock on hidden data.
ALLOWED_EMOJI = set("🟢🟡🔵🟠🔴⚫⚪✅⚠️❌⬜🔒")
...
@pytest.mark.parametrize("lang", list(CATALOGS))
def test_no_decorative_emoji_in_the_catalogs(lang):
    stray = {
        k: v for k, v in CATALOGS[lang].items()
        if set(EMOJI_RANGES.findall(v)) - ALLOWED_EMOJI
    }
    assert not stray, f"decorative emoji in {lang}: {stray}"
