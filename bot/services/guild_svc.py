...
Only the language lives here so far. It is read on nearly every render, so the
resolved value is cached in-process and invalidated on write — a board redraw
must not cost a database round trip per string.
"""
