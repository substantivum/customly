...
from bot.core.embeds import EMBED_COLOR, member_name
...
class StatsCog(commands.GroupCog, name="stats"):
    ...
    async def leaderboard(self, itx: discord.Interaction):
        ...
        if not top:
            return await reply(itx, t("stats.none"))
        lines = [
            t("stats.leader_line", rank=i + 1, name=member_name(itx.guild, p.user_id),
              wins=p.wins)
            for i, p in enumerate(top)
        ]
        e = discord.Embed(title=t("stats.leaderboard.title"), color=EMBED_COLOR)
        e.add_field(name=t("stats.top", n=len(top)), value="\n".join(lines), inline=False)
        await reply(itx, embed=e)
